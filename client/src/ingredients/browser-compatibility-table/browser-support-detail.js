...
export function BrowserSupportDetail({ index, browser, support, versionAdded, currentNoteId, onNotesClick, indexNote }) {
  ...
  if (support === undefined || (!support.notes && !support.flags && !support.prefix && !Array.isArray(support))) {
    return (
      <BrowserCompatibilityBlock key={index} browser={browser} versionAdded={versionAdded} elementType={'td'} />
    )
  } else {
    if (support.prefix) {
      noteType = "prefix";
  ...
  return (
    <BrowserCompatibilityBlock key={index} icon={noteType} browser={browser} versionAdded={versionAdded} elementType={"td"} index={`bc-history-${index}`} onNotesClick={onNotesClick} currentNoteId={currentNoteId}>
      ...
      {indexNote &&
        <div className="bc-notes"><BrowserSupportNotes key={`${indexNote.index}`} indexNote={indexNote} blockElementType="div" noteElementType="div" noBlocks /></div>
      }
